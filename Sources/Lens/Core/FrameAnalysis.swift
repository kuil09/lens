...
final class FrameAnalysis {
    ...
    func fingerprint(_ image: CIImage) -> FrameFingerprint {
        ...
        // Bitmap rows are top-first, while Vision bounds are bottom-left based.
        // Flip the rendered image so fingerprint row zero consistently means bottom.
        let small = image.transformed(by: CGAffineTransform(translationX: -image.extent.minX, y: -image.extent.minY))
