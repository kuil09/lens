...
struct LensLanguage: RawRepresentable, Hashable, Codable, Sendable, Identifiable {
    ...
    var title: String {
        L10n.locale.localizedString(forIdentifier: rawValue) ?? rawValue
    }
...
struct TextBlock: Identifiable, Sendable, Equatable {
    ...
    /// Physical OCR lines are retained separately from the translation input.
    let sourceLines: [SourceLine]
    var translationText: String {
        sourceLines.enumerated().reduce("") { result, entry in
            let text = entry.element.text.trimmingCharacters(in: .whitespacesAndNewlines)
            guard entry.offset > 0 else { return text }
            let separator = language == .japanese || result.hasSuffix("-") ? "" : " "
            return result + separator + text
        }
    }
    init(id: UUID = UUID(), text: String, bounds: CGRect, language: LensLanguage?, confidence: Float,
         sourceLines: [SourceLine]? = nil) {
        self.id = id; self.text = text; self.bounds = bounds
        self.language = language; self.confidence = confidence
        self.sourceLines = sourceLines ?? [.init(text: text, bounds: bounds)]
    }
}
