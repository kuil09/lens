...
final class LensBorder: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        for (inset, color) in [(CGFloat(0.5), NSColor.black.withAlphaComponent(0.65)),
            ...
            color.setStroke()
            let outline = NSBezierPath(rect: bounds.insetBy(dx: inset, dy: inset))
            outline.lineWidth = 1
            outline.stroke()
        }
    }
...
final class LensSurface: NSView {
    ...
    override init(frame: NSRect) {
        ...
        wantsLayer = true
        addSubview(canvas); addSubview(overlay); addSubview(border)
    }
    required init?(coder: NSCoder) { fatalError("init(coder:) has not been implemented") }
    override func layout() {
        super.layout()
        canvas.frame = bounds; overlay.frame = bounds; border.frame = bounds
