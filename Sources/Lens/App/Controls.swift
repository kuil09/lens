...
struct LensControls: View {
    @FocusState private var responsivenessFocused: Bool
    ...
    private var translation: some View {
        Group {
            ...
            Section {
                LabeledContent(L10n.text("Translation Refresh Responsiveness"), value: model.responsiveness.title)
                Slider(value: Binding(get: { Double(model.responsiveness.rawValue) }, set: {
                    model.responsiveness = TranslationResponsiveness(rawValue: Int($0.rounded())) ?? .balanced
                }), in: 0...2, step: 1) {
                    Text(L10n.text("Translation Refresh Responsiveness"))
                } minimumValueLabel: { Text(L10n.text("Calmer")) }
                  maximumValueLabel: { Text(L10n.text("Faster")) }
                .labelsHidden()
                .focusable()
                .focused($responsivenessFocused)
                .simultaneousGesture(TapGesture().onEnded { responsivenessFocused = true })
                .onKeyPress(.leftArrow) {
                    model.responsiveness = TranslationResponsiveness(rawValue: max(0, model.responsiveness.rawValue - 1)) ?? .balanced
                    return .handled
                }
                .onKeyPress(.rightArrow) {
                    model.responsiveness = TranslationResponsiveness(rawValue: min(2, model.responsiveness.rawValue + 1)) ?? .balanced
                    return .handled
                }
                .accessibilityValue(model.responsiveness.title)
                .help(L10n.text("Faster checks changes more often and may use more CPU and battery. This controls inspection waits, not translation completion time."))
            } footer: {
                Text(L10n.text("Faster checks changes more often and may use more CPU and battery. This controls inspection waits, not translation completion time."))
            }
        }
    }
