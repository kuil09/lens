...
@MainActor final class LensExportStore: ObservableObject {
    ...
    func openDirectory(using open: (URL) -> Bool) throws {
        let folder = try preparedDirectory()
        guard open(folder) else { throw ExportError.directoryOpenFailed }
    }
...
enum ExportError: LocalizedError {
    case directoryUnavailable, directoryOpenFailed, nameExhausted
    var errorDescription: String? {
        ...
        case .directoryOpenFailed: L10n.text("Cannot open the save folder in Finder. Try again or choose a folder again in Settings → Capture.")
