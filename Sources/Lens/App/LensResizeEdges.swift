...
@MainActor enum LensResizeChrome {
    ...
    static func draw(around pair: CGRect, recording: Bool = false) {
        let contour = recording
            ? NSBezierPath(roundedRect: pair.insetBy(dx: -outset, dy: -outset), xRadius: radius, yRadius: radius)
            : path(around: pair)
        NSColor(white: 0.08, alpha: 0.92).setStroke()
        contour.lineWidth = outlineWidth; contour.stroke()
        (recording ? NSColor.systemRed : NSColor(white: 1, alpha: 1)).setStroke()
...
@MainActor final class LensResizeHandle: NSView {
    let side: LensResizeSide
    ...
    func refreshAvailability() {
        let locked = owner?.recordingGeometryLocked == true
        let label = L10n.text(locked ? "Recording — stop recording to move or resize Lens." : "Resize Lens")
        setAccessibilityLabel(label)
        setAccessibilityEnabled(!locked)
        toolTip = label
        if locked { initial = nil }
        window?.invalidateCursorRects(for: self)
        needsDisplay = true
    }
    override var isOpaque: Bool { false }
    override func mouseDown(with event: NSEvent) {
        guard let owner, !owner.interactionSuspended, !owner.recordingGeometryLocked else { return }
    ...
    override func mouseDragged(with event: NSEvent) {
        guard let owner, let initial, !owner.interactionSuspended, !owner.recordingGeometryLocked, owner.isVisible else { return }
    ...
    override func resetCursorRects() {
        ...
        if owner.recordingGeometryLocked { addCursorRect(bounds, cursor: .arrow); return }
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        LensResizeChrome.draw(around: pair.offsetBy(dx: -strip.minX, dy: -strip.minY), recording: owner.recordingGeometryLocked)
    }
}
