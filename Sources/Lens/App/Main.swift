...
final class LensAppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, NSMenuItemValidation {
    ...
    private func showPermissionGuide() {
        ...
        if permissionWindow == nil {
            ...
            window.contentView = NSHostingView(rootView: LensPermissionGuide(state: onboarding,
                model: model, recording: recording,
    ...
    private func openExportDirectory() {
        do { NSWorkspace.shared.open(try exports.preparedDirectory()) }
        catch { showExportError(error) }
    }

    private func showExportError(_ error: Error) {
        let alert = NSAlert()
        alert.messageText = L10n.text("Could Not Save")
    ...
    func windowDidBecomeKey(_ notification: Notification) {
        ...
        if notification.object as? NSWindow !== lens || systemHandoff.isActive { lens.level = .normal }
    }
