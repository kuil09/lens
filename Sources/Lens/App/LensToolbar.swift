...
@MainActor final class LensToolbar: NSObject, NSToolbarDelegate {
    ...
    func toolbarDefaultItemIdentifiers(_ toolbar: NSToolbar) -> [NSToolbarItem.Identifier] {
        [.init("capture"), .init("record"), .flexibleSpace, .init("lock"), .init("reader"), .init("settings")]
    }
    func toolbarAllowedItemIdentifiers(_ toolbar: NSToolbar) -> [NSToolbarItem.Identifier] { toolbarDefaultItemIdentifiers(toolbar) }
    func toolbar(_ toolbar: NSToolbar, itemForItemIdentifier identifier: NSToolbarItem.Identifier, willBeInsertedIntoToolbar flag: Bool) -> NSToolbarItem? {
        guard ["capture", "record", "lock", "reader", "settings"].contains(identifier.rawValue) else { return nil }
        let item = NSToolbarItem(itemIdentifier: identifier)
        item.target = self; item.action = #selector(performAction(_:))
        item.autovalidates = false
        item.isBordered = true
        item.visibilityPriority = identifier.rawValue == "record" ? .high : .standard
    ...
    func update(_ state: LensToolbarState) {
        self.state = state
        configure("capture", label: L10n.text("Capture"), symbol: "camera", enabled: state.hasFrame && !state.choosingDestination,
                  help: state.hasFrame ? L10n.text("Save a PNG directly to your chosen folder. ⇧⌘S") : L10n.text("Connect the screen to save an image."))
        configure("record", label: state.recording ? L10n.text("Stop Recording") : (state.finishing ? L10n.text("Saving…") : L10n.text("Start Recording")),
                  ...
                  help: state.recording ? L10n.text("Stop and save an MP4 to your chosen folder. ⇧⌘R") : L10n.text("Start recording now. Silent video is saved to your chosen folder. ⇧⌘R"))
