...
@MainActor final class LensPanel: NSPanel, NSWindowDelegate {
    ...
    private(set) var recordingGeometryLocked = false
    private var recordedPairFrame: CGRect?
    var recordingGeometryUnchanged: Bool { recordingGeometryLocked && recordedPairFrame == pairFrame }
    private var geometryBeforeRecording: (movable: Bool, resizable: Bool, bodyResizable: Bool)?
    ...
    func setRecordingGeometryLocked(_ locked: Bool) {
        guard locked != recordingGeometryLocked else { return }
        recordingGeometryLocked = locked
        if locked {
            recordedPairFrame = pairFrame
            geometryBeforeRecording = (isMovable, styleMask.contains(.resizable), bodyPanel?.styleMask.contains(.resizable) == true)
            isMovable = false
            styleMask.remove(.resizable)
            bodyPanel?.styleMask.remove(.resizable)
            edgeResizing = false
        } else if let previous = geometryBeforeRecording {
            isMovable = previous.movable
            if previous.resizable { styleMask.insert(.resizable) }
            if previous.bodyResizable { bodyPanel?.styleMask.insert(.resizable) }
            geometryBeforeRecording = nil
            recordedPairFrame = nil
        }
        standardWindowButton(.zoomButton)?.isEnabled = !locked
        synchronizeEdges()
        resizePanels.forEach { ($0.contentView as? LensResizeHandle)?.refreshAvailability() }
    }
    ...
    func beginEdgeResize() {
        guard !interactionSuspended, !recordingGeometryLocked else { return }
        edgeResizing = true
    ...
    private func hideAuxiliaries(_ sender: Any?) {
        edgeResizing = false
        (bodyPanel?.contentView as? LensSurface)?.overlay.resetTransitions()
        (bodyPanel?.contentView as? LensSurface)?.overlay.dismissPopover()
    ...
    override func zoom(_ sender: Any?) {
        guard !recordingGeometryLocked, let screen else { return }
        onBodyArrangement?(false)
        let target: CGRect
