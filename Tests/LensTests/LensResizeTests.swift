...
@Test @MainActor func recordingLocksGeometryButKeepsControlsAndSecurityHide() throws {
    ...
    defer { lens.close() }
    lens.setBodyClickThrough(true)
    lens.orderFrontRegardless()
    let original = lens.pairFrame
    var arrangements = 0
    lens.onBodyArrangement = { _ in arrangements += 1 }
    lens.setRecordingGeometryLocked(true)
    lens.setRecordingGeometryLocked(true)
    #expect(!lens.isMovable && !lens.styleMask.contains(.resizable))
    #expect(lens.bodyPanel?.styleMask.contains(.resizable) == false)
    #expect(!lens.ignoresMouseEvents && lens.bodyPanel?.ignoresMouseEvents == true)
    #expect(lens.standardWindowButton(.zoomButton)?.isEnabled == false)
    lens.beginEdgeResize()
    lens.resizePair(to: original.insetBy(dx: -50, dy: -50))
    lens.zoom(nil)
    lens.endEdgeResize()
    #expect(lens.pairFrame == original && arrangements == 0 && !lens.edgeResizing)
    lens.interactionSuspended = true
    lens.orderOut(nil)
    #expect(lens.resizePanels.allSatisfy { !$0.isVisible })
    #expect(lens.bodyPanel?.isVisible == false)
    lens.setRecordingGeometryLocked(false)
    #expect(lens.isMovable && lens.styleMask.contains(.resizable))
    #expect(lens.bodyPanel?.styleMask.contains(.resizable) == true)
    lens.interactionSuspended = false
    lens.resizePair(to: original.insetBy(dx: -20, dy: -20))
    #expect(lens.pairFrame != original)
}
@Test @MainActor func roundedResizeCornersStayContinuousAcrossInputStrips() throws {
    ...
    for scale in [CGFloat(1), CGFloat(2)] {
        func render(strips: Bool, dark: Bool? = nil) throws -> (CGImage, [UInt8]) {
            ...
            } else { LensResizeChrome.draw(around: pair, recording: lens.recordingGeometryLocked) }
        ...
        lens.setRecordingGeometryLocked(true)
        let (_, recordingContinuous) = try render(strips: false)
        let (_, recordingSplit) = try render(strips: true)
        #expect(zip(recordingContinuous, recordingSplit).allSatisfy { abs(Int($0) - Int($1)) <= 1 })
        #expect(recordingSplit != split)
        for dark in [false, true] {
            let (image, _) = try render(strips: true, dark: dark)
            let png = try #require(NSBitmapImageRep(cgImage: image).representation(using: .png, properties: [:]))
            try png.write(to: output.appendingPathComponent("recording-outline-\(dark ? "dark" : "light")-\(Int(scale))x.png"))
        }
        lens.setRecordingGeometryLocked(false)
    }
}
