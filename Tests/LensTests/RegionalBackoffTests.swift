...
@Test func regionalBackoffHasBoundedPeriodicChecksAndEarlyStableRecovery() {
    ...
    let cell = 2 * 8 + 7
    #expect(scheduler.regions[cell].interval(for: scheduler.responsiveness) == 2)
    #expect(attempts.count >= 2 && attempts.count <= 4)
    #expect(zip(attempts, attempts.dropFirst()).allSatisfy { $1 - $0 <= 2.11 })
    #expect(scheduler.regions[0].interval(for: scheduler.responsiveness) == 0.25 && scheduler.regions[0].attempts == 1)
    ...
    #expect(scheduler.regions[cell].interval(for: scheduler.responsiveness) == 0.25)
...
@Test(arguments: TranslationResponsiveness.allCases) @MainActor func numericNegationChangeRejectsLateResultAndNeverFuzzilyReusesIt(profile: TranslationResponsiveness) async {
    let h = Harness()
    h.pipeline.setResponsiveness(profile)
...
@Test @MainActor func responsivenessChangeRetainsValidResultsAndDoesNotCancelActiveTranslation() async {
    let h = Harness()
    await h.initial()
    let display = h.pipeline.display, epoch = h.pipeline.scheduler.epoch
    h.pipeline.setResponsiveness(.fast)
    #expect(h.pipeline.display == display && h.pipeline.scheduler.epoch == epoch)
    h.blocks.append(paragraph("A new paragraph.", bounds: changingBounds))
    h.feed(at: 0.4, sample([changingPixel])); h.time.value = 0.53
    #expect(await h.pipeline.recognizeNext())
    h.engine.action = { [unowned h] in h.pipeline.setResponsiveness(.calm) }
    #expect(await h.pipeline.translateNext())
    #expect(h.pipeline.display.count == 2 && h.engine.cancellations == 0)
    #expect(h.pipeline.display.contains(display[0]))
    h.engine.action = nil
}
...
@Test(arguments: TranslationResponsiveness.allCases) @MainActor func boundedWorkersUseLatestFrameAndNeverCatchUpInABurst(profile: TranslationResponsiveness) async {
    let h = Harness()
    h.pipeline.setResponsiveness(profile)
    h.feed(at: 0); h.time.value = 0.13
...
@Test(arguments: TranslationResponsiveness.allCases) @MainActor func allDueRegionsShareOneObservationAndTranslationBatchesStayBounded(profile: TranslationResponsiveness) async {
    let h = Harness()
    h.pipeline.setResponsiveness(profile)
...
@Test func continuousBroadChangesDoNotResetTheirDeadlineForever() {
    ...
    #expect(scheduler.regions.allSatisfy { $0.interval(for: scheduler.responsiveness) == 2 })
}
