...
check_repo() {
    ...
    python3 -B "$ROOT/scripts/tests/ci-tests.py"
    python3 -B "$ROOT/scripts/tests/ci-success-gate.py"
    python3 -B "$ROOT/scripts/tests/documentation.py"
    printf 'Lens checks passed.\n'
}

clean_outputs() {
    local name target
    local names=(Build CompilationCache.noindex Logs ModuleCache.noindex SDKExplicitPrecompiledModules SDKStatCaches.noindex Index.noindex SourcePackages SwiftPM info.plist .DS_Store)
...
case "$COMMAND" in
    test)
        ...
        # Serialize the complete suite; hosted-runner comparisons stalled in parallel.
        LENS_TEST_INSTALLED_LANGUAGES=$INSTALLED swift test --no-parallel --package-path "$ROOT" --scratch-path "$DD/SwiftPM" ;;
    build)
