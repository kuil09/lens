...
usage() {
    printf '%s\n' \
        ...
        '  --development         package only; explicitly nonnotarized preview ZIP' \
...
check_repo() {
    local file bad=0 listing
    while IFS= read -r -d '' file; do bash -n "$file"; done < <(find "$ROOT/scripts" -type f -name '*.sh' -print0)
    git -C "$ROOT" diff --check
    git -C "$ROOT" diff --cached --check
    listing=$(git -C "$ROOT" ls-files) || die 'Cannot inspect tracked files.'
    while IFS= read -r file; do
        case "$file" in
            .build/*|build/*|build-*/*|DerivedData/*|dist/*|*.app/*|*.dSYM/*|*.xcarchive/*|*.xcuserstate|*/xcuserdata/*|.DS_Store|*/.DS_Store|.env|.env.*|*/.env|*/.env.*|*.p12|*.p8|*.keychain|*.keychain-db)
                printf 'Tracked generated/private artifact: %s\n' "$file" >&2; bad=1 ;;
        esac
    done <<< "$listing"
    [ "$bad" = 0 ] || die 'Tracked artifact hygiene failed.'
...
package_app() {
    ...
    safe_path "$dist"
    archive="$dist/Lens-$VERSION-$CHANNEL-$BUILD_NUMBER$suffix.zip"
...
APP=''; DEVELOPMENT=0; CONFIRMED_ID=''; INSTALLED=0
while [ "$#" -gt 0 ]; do
    case "$1" in
        --derived-data|--app|--confirmed-distribution-id)
...
case "$COMMAND" in
    ...
    build)
        ...
        xcodebuild -project "$ROOT/Lens.xcodeproj" -scheme Lens -configuration Release \
            ...
            CODE_SIGN_STYLE=Manual CODE_SIGN_IDENTITY=- PRODUCT_BUNDLE_IDENTIFIER=dev.local.lens \
