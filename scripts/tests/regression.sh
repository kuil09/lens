...
expect_fail 'Format must be zip or dmg' package --format exe
expect_fail 'Format is package-only' build --format dmg
...
# Consume the whole listing: grep -q can SIGPIPE unzip under pipefail.
unzip -Z1 "$PREVIEW" > "$WORK/archive-entries"
grep -qx LICENSE "$WORK/archive-entries"
grep -qx Lens.app/Contents/Info.plist "$WORK/archive-entries"
(cd "$REPO/dist" && shasum -a 256 -c "${PREVIEW##*/}.sha256") > "$WORK/result"
expect_fail 'refusing overwrite' package --development
COUNT=$((COUNT + 1))
mkdir -p "$ALT/Build" "$ALT/Unrelated" "$ALT/SwiftPM/arm64-apple-macosx/debug" "$ALT/SDKExplicitPrecompiledModules" "$REPO/build-historical"
cp "$FIXTURES/Info.plist" "$ALT/Build/generated.plist"
cp "$FIXTURES/Info.plist" "$ALT/Unrelated/keep.plist"
cp "$FIXTURES/Info.plist" "$ALT/SDKExplicitPrecompiledModules/generated.plist"
ln -s arm64-apple-macosx/debug "$ALT/SwiftPM/debug"
ln -s "$ALT/Unrelated" "$ALT/Build/external-link"
bash "$SCRIPT" clean --derived-data "$ALT" > "$WORK/result" 2>&1
test ! -e "$ALT/Build"
test ! -e "$ALT/SwiftPM"
test ! -e "$ALT/SDKExplicitPrecompiledModules"
