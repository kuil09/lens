...
7. Compute SHA-256 after stapling. Output only the DMG, checksum, and allowlisted source/submission provenance. A source-SHA suffix distinguishes CI artifacts from the already-published beta.5 even while version metadata is unchanged. Bump version/build/channel before a new public release.
...
On 2026-09-16, [run 35045717231](https://github.com/kuil09/lens/actions/runs/35045717231) at `9a8a5ca` passed checks/tests, certificate import, signing, Apple authentication, and app upload. Apple rejected app submission `41a36ff7-547d-4df6-80e8-e52da07a4450` as `Invalid` because the executable requested `com.apple.security.get-task-allow`. No DMG was submitted and no artifact was uploaded. The CI build had omitted the base-entitlement injection override present in the ignored local configuration. The explicit override and pre-submission binary check address this workflow defect; a fresh protected CI run must prove notarization success. Existing locally notarized build 15 release assets are unaffected and must remain in place until the CI replacement passes verification.
