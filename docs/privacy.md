...
Captured frames, recognized text, and translations are held in memory for display and processing. A bounded in-memory translation cache may retain text during the app session; pausing is not a promise to erase that cache. Lens stores preferences and window state, not an automatic translation-history archive.
